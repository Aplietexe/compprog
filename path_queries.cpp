struct PathQueries {  // O(n log n) preprocessing, O(log n) query
  typedef ll T; constexpr static T NEUT = -1e18;
  T op(const T &a, const T &b) { return max(a, b); }  // associative and commutative
  ll n, K;
  vector<vi> &g, anc;  // remove & for edge queries or if instance lives longer
  ...
  T query(ll u, ll v) {
    ...
    if (u == v) return op(ans, part[0][u]);  // node queries
    for (ll k = K - 1; k >= 0; --k)
